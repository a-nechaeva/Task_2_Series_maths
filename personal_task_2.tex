...
Рассмотрим поведение ряда в точках $x = \frac{\pi m}{2}, \, m \in N$. В точках вида $x = \frac{\pi}{2} \cdot (2k + 1), \,\, k \in N$ ряд сходится абсолютно \hyperlink{pdf}{ (см. в начале решения)}, а сходимость ряда при четных значениях $m \in N$ доказана выше.\\
Область \textbf{условной} сходимости теперь  $x \in (0, + \infty) $.\\\\
...
\end{multline*}
Сумма рядов выше расходится при $x \leq 3$.\\

Далее, будем рассматривать $x > 3$.
\begin{multline*}
\left| \frac{\cos( nx) \cdot \cos (3nx) }{n^{\frac{x}{3}}}  \right| =  
\end{multline*}
\end{document}
